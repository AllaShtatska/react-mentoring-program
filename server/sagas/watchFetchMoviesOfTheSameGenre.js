...
const getFilterByTheSameGenrePart = (genres) => {
...
function* fetchMoviesOfTheSameGenre(action) {
    try {
        const movies = yield fetch('https://reactjs-cdp.herokuapp.com/movies?limit=5000' + getFilterByTheSameGenrePart(action.genres)).then(function (response) {
