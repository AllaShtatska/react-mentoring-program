...
describe("movieDetails reducer", () => {
  ...
  it("should set selected movie", () => {
    ...
    expect(movieDetails({ selectedMovie: null, moviesOfTheSameGenre: [] }, { type: SET_SELECTED_MOVIE, movie })).toEqual({
      selectedMovie: movie,
      moviesOfTheSameGenre: [],
    });
  });
  ...
  it("should set movies of the same genre", () => {
    ...
    expect(movieDetails({ selectedMovie: movies[0], moviesOfTheSameGenre: [] }, { type: MOVIES_OF_THE_SAME_GENRE_FETCH_SUCCEEDED, movies })).toEqual({
